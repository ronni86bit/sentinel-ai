            ...
            {navItems.map((item) => {
              ...
              return (
                <motion.button
                  ...
                  className={cn(
                    'w-full flex items-center space-x-3 px-3 py-2.5 rounded-xl text-xs font-medium transition-all cursor-pointer relative group min-h-[44px]',
                    isActive
                      ? 'bg-blue-50 dark:bg-blue-950/50 text-blue-700 dark:text-blue-400 font-semibold ring-1 ring-blue-500/20 shadow-xs'
...
<p className="text-[10px] text-zinc-500 dark:text-zinc-400 leading-normal">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 dark:bg-zinc-950/95 backdrop-blur-md border-t border-zinc-200 dark:border-zinc-800 flex items-center justify-around py-2 px-2 shadow-lg transition-colors">
        {navItems.map((item) => {
          const Icon = item.icon;
          const isActive = activeTab === item.id;
          return (
            <motion.button
              ...
              onClick={() => setActiveTab(item.id)}
              className={cn(
                'flex flex-col items-center justify-center py-1.5 px-3 rounded-xl text-xs transition-all cursor-pointer min-w-[64px] min-h-[44px]',
                isActive
